...
output_file = f"rcdata_{current_time}.csv"
...
async def main():
    ...
    def reconstruct_csv(input_file, output_file):
        with open(input_file, 'r') as infile, open(output_file, 'w', newline='') as outfile:
            reader = csv.reader(infile)
            writer = csv.writer(outfile)

            for row in reader:
                if len(row) < 37:  # Skip incomplete rows
                    continue
                ...
                # Reshape the data into 5 lines
                for i in range(5):
                    new_row = [device_name, time, ids[i]]
                    new_row.extend(data[i*6:(i+1)*6])
    ...
    while True:
        ...
        if data.decode('utf-8').lower() == "disconnect":
            ...
            reconstruct_csv(filename, output_file)
            break
